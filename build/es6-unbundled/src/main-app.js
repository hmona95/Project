define(["../node_modules/@polymer/polymer/polymer-element.js","../node_modules/@polymer/paper-dialog/paper-dialog.js","./modal-contents.js","./kanban-container.js","../node_modules/@polymer/iron-ajax/iron-ajax.js"],function(_polymerElement,_paperDialog,_modalContents,_kanbanContainer,_ironAjax){"use strict";class MainApp extends _polymerElement.PolymerElement{static get template(){return _polymerElement.html`
    ...
    `}openModal(){this.$.modal.open()}ready(){super.ready();this.$.taskInputs.addEventListener("new task",event=>this.addTask(event));this.$.kanban.addEventListener("status change",event=>this.statusChange(event));this.socket=io();this.socket.on("task added",data=>{this.push("tasks",data.payload)});this.socket.on("task updated",data=>{this.getAll()})}addTask(event){this.id="";this.set("body",event.detail);this.$.dataAjax.method="POST";this.$.modal.close()}statusChange(event){this.set("body",{status__c:event.detail.status__c});this.id=event.detail.id;this.$.dataAjax.method="PUT"}getAll(){this.id="";if("GET"===this.$.dataAjax.method){this.$.dataAjax.generateRequest()}else{this.$.dataAjax.method="GET"}}handleResponse(event,res){if("GET"===this.$.dataAjax.method){this.set("tasks",res.response)}else{this.getAll()}}static get properties(){return{tasks:Array,body:Object,url:String,id:String}}constructor(){super();this.url="/api/tasks";this.id=""}}customElements.define("main-app",MainApp)});